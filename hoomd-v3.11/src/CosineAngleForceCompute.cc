...
void CosineAngleForceCompute::setParams(unsigned int type, Scalar K, Scalar t_0)
    ...
    if (t_0 < 0)
        m_exec_conf->msg->warning() << "angle.cosine: specified t_0 < 0" << endl;
    }
...
void CosineAngleForceCompute::computeForces(uint64_t timestep)
	...
	*/
	...
	Scalar rsqbc = dbc.x * dbc.x + dbc.y * dbc.y + dbc.z * dbc.z; // squared magnitude of r_bc
        Scalar rbc = sqrt(rsqbc);                                     // magnitude of r_bc
	...
	*/
	...
	Scalar c_abbc = dab.x * dbc.x + dab.y * dbc.y + dab.z * dbc.z; // = ab dot bc
	c_abbc /= rab * rbc;                                           // cos(t)


        if (c_abbc > 1.0)
            c_abbc = 1.0; // how does this ever happen?
        if (c_abbc < -1.0)
            c_abbc = -1.0;
	...
	*/
	unsigned int angle_type = m_angle_data->getTypeByIndex(i);
	Scalar t_0=m_t_0[angle_type]; //TM 2022/12/13
	Scalar K=m_K[angle_type];     //TM 2022/12/13
	Scalar s_abbc  =Scalar(1.0);  //TM 2022/12/13
	Scalar cot_abbc=Scalar(0.0);  //TM 2022/12/13
	Scalar c0=Scalar(1.0);        //TM 2022/12/13
	Scalar s0=Scalar(0.0);        //TM 2022/12/13
	s_abbc = sqrt(1.0-c_abbc*c_abbc);   //TM 2022/12/15
	//if (s_abbc < SMALL) s_abbc = SMALL; //TM 2022/12/15
	//s_abbc = 1.0 / s_abbc;              //TM 2022/12/15
	if (s_abbc < SMALL){
	...
	c0=cos(t_0);
	s0=sin(t_0);
	Scalar dcosth = c_abbc*c0 + s_abbc*s0;// = cos(t - t0) = cos(t)cos(t0) + sin(t)sin(t0) :TM 2022/12/15
	Scalar tk = K*(1.0 - dcosth);      // = k(1 - cos(t-t0))            :TM 2022/12/15
	Scalar a = K*(c0 - cot_abbc*s0);// a = k(cos(t0) - cot(t)sin(t0)) :TM 2022/12/15
	Scalar a11 = -a * c_abbc / rsqab;// = -a cos(t)/ rab^2             :TM 2022/12/15

	/*
	Scalar a12 = +a / (rab * rcb);   // = a / (rab rcb)                :TM 2022/12/15
	Scalar a22 = -a * c_abbc / rsqcb;// = -a cos(t)/ rcb^2             :TM 2022/12/15
	*/

	Scalar a12 = +a / (rab * rbc);   // = a / (rab rbc)                :TM 2022/12/15
	Scalar a22 = -a * c_abbc / rsqbc;// = -a cos(t)/ rbc^2             :TM 2022/12/15
	...
	*/
	Scalar angle_eng = tk*Scalar(1.0/3.0);  //TM 2022/12/13
	...
	angle_virial[0] = Scalar(1. / 3.) * (dab.x * fab[0] + dbc.x * fbc[0]);
        ...
        if (idx_b < m_pdata->getN())
	      ...
	      */
	      ...
	      h_force.data[idx_b].x -= fab[0] + fbc[0];
	      h_force.data[idx_b].y -= fab[1] + fbc[1];
	      h_force.data[idx_b].z -= fab[2] + fbc[2];
        ...
        if (idx_c < m_pdata->getN())
	      ...
	      */
	      h_force.data[idx_c].x += fbc[0];
	      h_force.data[idx_c].y += fbc[1];
	      h_force.data[idx_c].z += fbc[2];
