...
class cosine(force._force):
    R""" Cosine angle potential.

    The command angle.cosine specifies a cosine potential energy
    between every triplet of particles with an angle specified between them.

    .. math::

        V(\theta) = k \left( 1 - \cos(\theta - \theta_0) \right)
    ...
    Coefficients :math:`k` and :math:`\theta_0` must be set for each type of
    angle in the simulation using the method :py:math:`angle_coeff.set() <coeff.set()>`.
    Note that the value of :math:`k` for this angle potential is not comparable to
    the value of :math:`k` for harmonic angles, as they have different units.
    ...
    Examples::
        ...
        cosine = angle.cosine()
        cosine.angle_coeff.set('polymer', k=3.0, t0=0.7851)
        cosine.angle_coeff.set('backbone', k=100.0, t0=1.0)
    ...
    def __init__(self):
        ...
        # create the c++ mirror class
        if not hoomd.context.exec_conf.isCUDAEnabled():
            self.cpp_force = _md.CosineAngleForceCompute(
                    hoomd.context.current.system_definition);
        else:
            self.cpp_force = _md.CosineAngleForceComputeGPU(
                    hoomd.context.current.system_definition);
