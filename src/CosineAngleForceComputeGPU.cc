  ...
  : CosineAngleForceCompute(sysdef)
{
  ...
  m_tuner.reset(new Autotuner(32, 1024, 32, 5, 100000, "cosine_angle", this->m_exec_conf));
}
...
*/
void CosineAngleForceComputeGPU::computeForces(unsigned int timestep)
{
  // start the profile
  if (m_prof) m_prof->push(m_exec_conf, "Cosine Angle");
  ...
  m_tuner->begin();
  gpu_compute_cosine_angle_forces(d_force.data,
				  d_virial.data,
				  m_virial.getPitch(),
				  m_pdata->getN(),
				  d_pos.data,
...
void export_CosineAngleForceComputeGPU(py::module& m)
{
  py::class_<CosineAngleForceComputeGPU, std::shared_ptr<CosineAngleForceComputeGPU> >(m, "CosineAngleForceComputeGPU", py::base<CosineAngleForceCompute>())
